...
class Runner(Node):
    ...
    def set_opc_callback(self, msg: String):
        ...
        try:
            ...
            fixed = {k.replace(
                "ns=4;s=|var|CODESYS CONTROL FOR Raspberry Pi MC SL.Application.IO.", ""): v for k, v in kvs.items()}
    ...
    def locked_done_callback(self, future):
        ...
        if not result:
            print(
                f"The locked service did not like the call. Check log in simulator window")
...
def tick_the_runner(state: State, model: Model, pre_start: bool) -> State:
    ...
    if current_op_state == "i" and current_op.eval_run(state):
        next_state = current_op.start(state)
    elif current_op_state == "i":  # the operation should be started but is not enabled
        next_state = state.next(
            plan_status=f"waiting for op {current_op_name} to be enabled. pre: {current_op.precondition} and {current_op.to_run}")
    # the operation has completed and we can take a step in the plan
    elif current_op.is_completed(state):
        next_state = current_op.complete(state)
        next_state = next_state.next(
            step_in_plan=next_step, plan_status=f"completing step {current_step_in_plan}")
...
def tick_the_random_runner(state: State, model: Model) -> State:
    ...
    running_ops: List[Operation] = [o for name,
                                    o in model.operations.items() if state.get(name) == "e"]
    next_state = state

    if not running_ops:
        enabled_ops = [o for _, o in model.operations.items()
                       if o.precondition.eval_run(state)]
        if not enabled_ops:
        ...
        ops_can_complete = [
            o for o in running_ops if o.postcondition.eval_run(state)]
        if ops_can_complete:
            next_state = state
